...
Wraps a trained PPO :class:`src.models.policy.Policy` behind a single interface
the Jacobian / fixed-point / ablation phases use:
...
class Policy2DAdapter:
    ...
    def step_stochastic(
        ...
        Mirrors how the PPO agent acts at training time:
        ``Normal(mean, sigma).sample()`` then clamp to the action box.
